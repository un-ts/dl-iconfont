...
(async() => {
  ...
  const newViewport = {
    width: Math.max(VIEWPORT.width, Math.ceil((boundingBox as puppeteer.BoundingBox).width)),
    height: Math.max(VIEWPORT.height, Math.ceil((boundingBox as puppeteer.BoundingBox).height)),
  };
